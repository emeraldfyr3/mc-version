...
execute at @e[type=minecraft:item,nbt={Item:{tag:{display:{Name:"{\"text\":\"mcversion\"}"}}}},limit=1] run kill @e[type=minecraft:armor_stand,tag=mcversion]
execute at @e[type=minecraft:item,nbt={Item:{tag:{display:{Name:"{\"text\":\"mcversion\"}"}}}},limit=1] unless score #1_16_3__loaded mcversion matches 1 run forceload remove 357704 1025512
